...
(function($) {
    ...
    $(document).ready(function() {
        ...
        // Filter changes
        $('#filter-status, #filter-category, #filter-source').on('change', function() {
    ...
    function refreshClaims() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    showSuccess('Claims refreshed successfully!');
                    loadClaims();
                } else {
            ...
            error: function() {
                showError('Failed to refresh claims. Please try again.');
            },
            complete: function() {
    ...
    function renderClaimRow(claim) {
        ...
        const truncatedClaim = claimText.length > 200 ? claimText.substring(0, 200) + '...' : claimText;
        ...
        let sourceDisplay = source.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase());
        ...
        if (status === 'pending') {
            ...
            actions = `
                <a href="${getEditUrl(claim.post_id)}" class="btn-view" target="_blank">
                    View Draft
                </a>
                <button class="btn-dismiss" data-claim-id="${claim.id}">
                    Dismiss
            ...
            actions = '<span style="color: #9ca3af; font-size: 12px;">Dismissed</span>';
        }
        ...
        return $(`
            <tr data-claim-id="${claim.id}">
                <td class="column-claim">
                    ...
                    ${claim.source_url ? `<div class="claim-meta"><a href="${escapeHtml(claim.source_url)}" target="_blank" style="color: #3b82f6;">View Source</a></div>` : ''}
                </td>
    ...
    function attachActionHandlers() {
        ...
        // Dismiss claim
        $('.btn-dismiss').off('click').on('click', function() {
            const claimId = $(this).data('claim-id');
